...
exports.coachBrainV2 = coachBrainV2;
...
function clamp(s, max = 520) {
    const x = s.trim();
    return x.length > max ? x.slice(0, max - 1).trim() + "…" : x;
}
function detectIntent(tRaw) {
    const t = norm(tRaw || "");
    if (!t)
        return "general";
    if (hasAny(t, ["hi", "hey", "yo", "hello", "wyd", "sup"]))
        return "greeting";
    if (hasAny(t, ["ghost", "left on read", "not replying", "dry", "ignoring", "not texting"]))
        return "no_reply";
    if (hasAny(t, ["ask her out", "ask him out", "date", "link", "hang", "pull up", "meet up"]))
        return "ask_out";
    if (hasAny(t, ["what do i say", "what should i text", "how do i reply", "what do i text back", "respond"]))
        return "reply_help";
    if (hasAny(t, ["what are we", "exclusive", "situationship", "relationship", "serious"]))
        return "define";
    if (hasAny(t, ["argue", "fight", "mad", "upset", "disrespect", "attitude"]))
        return "conflict";
    if (hasAny(t, ["broke up", "left me", "dumped", "breakup", "ended it"]))
        return "breakup";
    if (hasAny(t, ["sorry", "apologize", "my fault"]))
        return "apology";
    if (hasAny(t, ["flirt", "rizz", "smooth", "compliment", "game"]))
        return "flirt";
    return "general";
}
function coachStyle(mode) {
    if (mode === "rizz")
        return { openers: ["Say less 😌", "Bet — keep it smooth.", "Aight, short and bold."], vibeWords: ["smooth", "confident"] };
    return { openers: ["Got you.", "Okay — I hear you.", "That’s real."], vibeWords: ["calm", "direct"] };
}
function buildQuickReplies(mode, intent) {
    const dating = {
...
function historyText(sessionId) {
    return (0, memoryStore_1.getHistory)(sessionId).map((x) => `${x.role}:${x.text}`).join("\n");
}
function alreadyAsked(h, marker) {
...
function badReply(s) {
    ...
    return (lower.includes("that's real") ||
        lower.includes("aight") ||
        lower.includes("say less") ||
        lower.includes("tell me one sentence") ||
        (s || "").length < 20);
}
async function llmAssist(opts) {
    const { mode, userMessage, sessionId, intent, advanced = false } = opts;
    const h = (0, memoryStore_1.getHistory)(sessionId).slice(-12);
...
`;
    ...
    const sessionMem = (0, memoryStore_1.getSessionMemory)(sessionId || "") || {};
    const memLines = [];
    if (sessionMem.whoEndedIt)
        memLines.push(`whoEndedIt: ${sessionMem.whoEndedIt}`);
    if (sessionMem.why)
        memLines.push(`why: ${sessionMem.why}`);
    if (sessionMem.whatHappened)
        memLines.push(`whatHappened: ${sessionMem.whatHappened}`);
    if (sessionMem.userGoal)
        memLines.push(`userGoal: ${sessionMem.userGoal}`);
    if (typeof sessionMem.wantsClosure === "boolean")
        memLines.push(`wantsClosure: ${sessionMem.wantsClosure}`);
    ...
    const raw = await (0, llmOllama_1.ollamaChat)({ model, messages, temperature: advanced ? 0.35 : 0.25, maxTokens });
    return raw;
}
async function coachBrainV2(body) {
    const sessionId = body.sessionId;
    const advanced = !!body.advanced;
    const mode = body.mode === "rizz" ? "rizz" : body.mode === "strategy" ? "strategy" : "dating_advice";
    ...
    if (useLLM) {
        try {
            ...
            if (parsed && typeof parsed.reply === 'string') {
                ...
                if (badReply(parsed.reply)) {
                    try {
                        ...
                        if (parsed2 && typeof parsed2.reply === 'string') {
                            ...
                            return { message: reply, ...(parsed2 ? { coach: parsed2 } : {}) };
                        }
                    }
                    catch (e) {
                ...
                return { message: reply, ...(parsed ? { coach: parsed } : {}) };
            }
    ...
    if (!out)
        return { message: "Sorry, I couldn't generate a reply right now." };
    return { message: out };
}
