...
const adviceStore_1 = require("../store/adviceStore");
const coachScaffold_1 = require("../coachScaffold");
const entitlements_1 = require("../entitlements");
const upstash_1 = require("../upstash");
const memoryStore_1 = require("../memoryStore");
const router = (0, express_1.Router)();
exports.adviceRouter = router;
const adviceSchema = zod_1.z.object({
    author: zod_1.z.string().min(1),
    text: zod_1.z.string().min(1),
    tags: zod_1.z.array(zod_1.z.string()).optional()
});
router.get('/', (_req, res) => {
    res.json((0, adviceStore_1.getAllAdvice)());
});
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        let token = '';
        try {
            const tokenFromCookie = (req.cookies && req.cookies.ae_token) || req.headers['x-session-id'] || req.body?.sessionId || req.body?.uid;
            token = String(tokenFromCookie || '');
            requestToken = token || null;
            if (token) {
                const paid = await (0, upstash_1.isTokenPaid)(token);
                if (!paid) {
                    const used = await (0, upstash_1.getWeeklyUsage)(token);
                    if (used !== null && used >= 3) {
    ...
    if (looksLikeCreate) {
        ...
        const created = (0, adviceStore_1.addAdvice)(parsed.data);
    ...
    try {
        ...
        function analyzeRouter(inp, conv) {
            ...
            if (/ghost|left on read|no reply|not replying/.test(t))
                facts.push('no_reply');
            if (/cheat|cheated|cheating/.test(t))
                facts.push('possible_cheating');
            if (/sorry|apolog(y|ize)/.test(t))
                facts.push('apology');
            ...
            if (!facts.length)
                assumptions.push('no_major_flags_detected');
            return {
                intent,
        ...
        function coachResponse(_routerOut, _inp, _conv, _premium = false) {
        ...
        try {
            ...
            ent = (0, entitlements_1.getEntitlements)(uid);
            const remaining = (0, entitlements_1.getDailyRemaining)(uid).dailyRemaining;
            const weeklyRemaining = (0, entitlements_1.getWeeklyRemaining)(uid).weeklyRemaining;
            ...
            if (process.env.USE_OLLAMA === 'true') {
                try {
                    ...
                    const llmResp = await (0, coachScaffold_1.runCoach)({ sessionId: uid || '', userMessage: text, explicitMode: mode, advanced, model });
            ...
            if (process.env.USE_OLLAMA === 'true') {
                try {
                    const advanced = !!(ent && ent.isPremium);
                    const model = process.env.OLLAMA_MODEL || undefined;
                    const resp = await (0, coachScaffold_1.runCoach)({ sessionId: uid || '', userMessage: text, explicitMode: mode, advanced, model });
                    ...
                    try {
                        if (uid)
                            (0, memoryStore_1.pushTurn)(uid, { role: 'user', text, ts: Date.now() });
                    }
                    catch (e) {
                    ...
                    try {
                        if (uid)
                            (0, memoryStore_1.pushTurn)(uid, { role: 'coach', text: adviceText, ts: Date.now() });
                    }
                    catch (e) {
                    ...
                    try {
                        if (!(ent && ent.isPremium)) {
                            (0, entitlements_1.incrementConversationCount)(uid, 1);
                            try {
                                if (requestToken)
                                    (0, upstash_1.incrementWeeklyUsage)(requestToken);
                            }
                            catch (e) {
                    ...
                    try {
                        (0, memoryStore_1.setSessionMemory)(uid, { whatHappened: String(adviceText).slice(0, 600) });
                    }
                    catch (e) {
        ...
        try {
            if (uid)
                (0, memoryStore_1.pushTurn)(uid, { role: 'coach', text: (coachResult && coachResult.reply) || '', ts: Date.now() });
        }
        catch (e) {
            console.warn('push coach turn failed', e);
        }
        try {
            if (uid)
                (0, memoryStore_1.setSessionMemory)(uid, { whatHappened: ((coachResult && coachResult.reply) || '').slice(0, 600) });
        }
        catch (e) {
            console.warn('set session memory failed', e);
        }
