...
app.post('/api/chat', express.json(), async (req, res) => {
  try {
    ...
    let reply = '';
    try {
      if (resp && resp.message && (resp.message.content || resp.message.text)) {
        reply = typeof resp.message.content === 'string' ? resp.message.content : (resp.message.content ?? resp.message.text ?? '');
      } else {
        reply = extractTextFromOllamaResponse(resp) || '';
      }
    } catch (e) {
      reply = extractTextFromOllamaResponse(resp) || '';
    }
